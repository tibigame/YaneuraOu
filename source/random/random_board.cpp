...
Square set_b_king(Position& pos_) {
	...
	BoardInfo bi(sq, B_KING);
	list_bi.push_front(bi);
	return sq;
}; 
...
inline bool set_piece_core(
	...
	if (is_promoted_rand(sq, promoto_p)) { // 成り→不成の順で判定
		if (not_check_promote) { // 成りで王手にならない
			...
			not_promote = false;
			BoardInfo bi(sq, set_piece_promote);
			list_bi.push_front(bi);
			return true;
		}
...
bool _aigoma_b(Position& pos_, const Square sq,
	...
	if (hand_exists(pos_.hand[0], GOLD) && myrand.rand_b(select_p) && !(w_king_bit & GoldEffectBB[sq][BLACK])) {
	...
	if (hand_exists(pos_.hand[1], GOLD) && myrand.rand_b(select_p) && !(b_king_bit & GoldEffectBB[sq][WHITE])) {
...
bool _aigoma_w(Position& pos_, const Square sq,
	...
	if (hand_exists(pos_.hand[1], GOLD) && myrand.rand_b(select_p) && !(b_king_bit & GoldEffectBB[sq][WHITE])) {
		if (set_piece_core(pos_, sq, occupied, W_GOLD, W_GOLD, w_gold_promote_p,
			!(b_king_bit & GoldEffectBB[sq][WHITE]), !(b_king_bit & GoldEffectBB[sq][WHITE]), true)) {
			sub_hand(pos_.hand[1], GOLD);
			return true;
		}
	};
	if (hand_exists(pos_.hand[0], GOLD) && myrand.rand_b(select_p) && !(w_king_bit & GoldEffectBB[sq][BLACK])) {
		if (set_piece_core(pos_, sq, occupied, B_GOLD, B_GOLD, b_gold_promote_p,
			!(w_king_bit & GoldEffectBB[sq][BLACK]), !(w_king_bit & GoldEffectBB[sq][BLACK]), true)) {
			sub_hand(pos_.hand[0], GOLD);
			return true;
		}
	};
...
bool set_pawn(Position& pos_, const Square &b_king, const Square &w_king,
	...
	for (auto i = 0; i < b_board; ++i) {
		...
		// 二歩のときは一定確率、駒柱のときは再生成させる
		if ((!not_double_pawn && myrand.rand_b(0.6)) || ((occupied & FILE_BB[file]) == FILE_BB[file])) {
			--i;
